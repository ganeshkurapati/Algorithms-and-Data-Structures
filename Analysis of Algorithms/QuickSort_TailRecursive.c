...
void TAIL_RECURSIVE_QUICKSORT(int *At, int p, int r)
{
    while(p<r)
    {
        ...
        if((q-p) < (r-p))
        {
            TAIL_RECURSIVE_QUICKSORT(At,p,q-1);
            p = q+1;
        }
        else
        {
            TAIL_RECURSIVE_QUICKSORT(At,q+1,r);
            r = q-1;
        }
    }
}
...
int main()
{
    ...
    TAIL_RECURSIVE_QUICKSORT( A,0,(lengthof_A-1));
